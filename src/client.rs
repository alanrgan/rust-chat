...
impl WebSocketClient {
	...
	pub fn push_message(&mut self, s: String) {
		match self.state {
			ClientState::Connected => self.outgoing.push(WebSocketFrame::from(&s[..])),
			_ => {}
		}
	}
	...
	fn write_frames(&mut self) {
		...
		println!("sending {} frames to client {}", self.outgoing.len(), self.id);
		for (i,frame) in self.outgoing.iter().enumerate() {
	...
	pub fn new(socket: TcpStream, id: usize) -> WebSocketClient {
